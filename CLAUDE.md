...
1. Runs `elocuent -d . -o meta/loc.csv` to regenerate code statistics
2. Auto-commits `meta/loc.csv` back to the repo
...
- `meta/main.js` — scatterplot of commits by time-of-day, file-size dot plot, brushing + tooltips; data sourced from `meta/loc.csv` (generated by CI)

**Page pattern**: every subdirectory (`projects/`, `resume/`, `contact/`, `meta/`) has its own `index.html` and optionally a `.js` file. Pages at subdirectory depth adjust `src`/`href` paths with a `../` prefix.
