...
- **Verification.** Data layer checked by importing `data/site.js` in Node: `fmtDate` output for all three formats, `fmtRange` on existing experiences, blog and project sort order, and the Alongside entry's fields. The local server returns 200 for the changed pages. **Not visually verified in a browser this round** — the Chrome extension was disconnected, so the new tile, the day-level date strings, and the inline-code styling have not been seen rendered.
